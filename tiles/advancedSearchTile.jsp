...
<html:form styleId="searchForm" method="get" styleClass="EXLSearchForm"
           action="/action/search.do?${action_func}=search&amp;ct=search"
           enctype="application/x-www-form-urlencoded; charset=utf-8"
           onsubmit="if(isRemoteSearch()){doPleaseWait();}if(window.manualFormSubmit){manualFormSubmit(this.id);return false;}"
           target="${resloc}">
        ...
        <ul id="exlidSearchTabs" class="EXLTabs nav nav-tabs">
            <c:forEach items="${primoView.avilableTabs}" var="menu_item" varStatus="status">
                ...
                <li id="exlidTab${status.index}"
                    ...
                    <span id="defaultScope${menu_item}" style='display:none'><fmt:message
                            key='scopes.option.${defScopeId}'/></span> <%-- where is this used? (-hj) --%>
                    <a href="${tab_url}&tab=${menu_item}" data-toggle="tab"
                       class="EXLSearchTabTitle EXLSearchTabLABEL${menuItemLabel}"
                       onclick="getSearchField(this,'${searchForm.mode}'); delay4Remote('${isRemote}','${searchForm.tabsRemote} ','${menu_item}')"
					...
					<c:if test="${searchForm.location[queryTermStatus.index]=='L'}">
                        <c:choose>
                            <c:when test="${input.componentType == c_ctype_freetext}">
                                <label class="EXLHide collapse"
                                       for="input_${input.id}"><%-- accessibility instruction --%>
                                    Input search text:
                                </label>
                                <input type="text" class="form-control" id="input_${input.id}" name="vl(${input.id})"
                                       value="${fn:escapeXml(searchForm.values[input.id])}"/>
                            </c:when>
                            <c:otherwise>
                                <label class="EXLHide collapse" for="exlidInput_${input.componentType}_${count}">
                                    Show Results with:
                                </label>
                                <prm:select selectForm="${searchForm}" input="${input}" styleClass="fixed"
                                            valueOptionsPrefix='search-advanced' count="${count}" from="advance"/>
                            </c:otherwise>
                        </c:choose>
        ...
        <div class="EXLAdvancedSearchFormRow EXLAdvancedSearchFormRowPrefilters">
			...
			<span class="EXLAdvancedSearchFormRowInlineInput">
				<c:if test="${searchForm.showPeerReview}">
                    ...
                    <label for="exlidIncludePeerReviewed"><fmt:message
                            key="search-advanced.prefilter.peer_review"/></label>
                </c:if>
				<c:if test="${searchForm.showFullText}">
                    <html:checkbox styleId="exlidIncludeFullText" property="includeFullText"/>
                    <label for="exlidIncludeFullText"><fmt:message
                            key="search-advanced.prefilter.full_text_online"/></label>
                </c:if>
			</span>
            </fieldset>
        </div>
                    ...
                    <c:when test="${searchForm.termType[queryTermStatus.index]=='DateRange'}">
                        <div class="EXLAdvancedSearchFormRow">
                            <div class="EXLAdvancedSearchFormDateRangeRow form-group">
                                <label for="exlidInput_DateRange_Start"><fmt:message
                                        key="search-advanced.DateRange.label.DateRangeStart"/></label>
                                        ...
                                        <c:if test="${status.index < 3}">
                                            <c:choose>
                                                <c:when test="${input.componentType == 'drStartYear'}">
                                                    <label class="EXLHide hide" for="input_${input.id}">query
                                                        input</label>
                                                    <c:choose>
                                                        <c:when test="${fn:escapeXml(searchForm.values[input.id]) == ''}">
                                                            <div class="col-xs-4 col-md-4"><input type="text"
                                                                                                  class="form-control input-sm"
                                                                                                  id="input_${input.id}"
                                                                                                  name="vl(${input.id})"
                                                                                                  value="<fmt:message key="
                                                                                                  search-advanced.DateRange.label.Year"
                                                                />" />
                                                            </div>
                                                        </c:when>
                                                        <c:otherwise>
                                                            <div class="col-xs-4 col-md-4">
                                                                <input type="text" id="input_${input.id}"
                                                                       name="vl(${input.id})"
                                                                       value="${fn:escapeXml(searchForm.values[input.id])}"/>
                                                            </div>
                                                        </c:otherwise>
                                                ...
                                                <c:otherwise>
                                                    <div class="col-xs-4 col-md-4">
                                                        <prm:select selectForm="${searchForm}" input="${input}"
                                                                    styleClass="fixed"
                                                                    valueOptionsPrefix='search-advanced'/>
                                                    </div>
                                                </c:otherwise>
                                            </c:choose>
                                        </c:if>
                        ...
                        <div class="EXLAdvancedSearchFormRow">
                            <div class="EXLAdvancedSearchFormDateRangeRow form-group">
                                <label for="exlidInput_DateRange_Start"><fmt:message
                                        key="search-advanced.DateRange.label.DateRangeEnd"/></label>
                                                        ...
                                                        <c:when test="${fn:escapeXml(searchForm.values[input.id]) == ''}">
                                                            <div class="col-xs-4 col-md-4">
                                                                <input type="text" class="form-control input-sm"
                                                                       id="input_${input.id}" name="vl(${input.id})"
                                                                       value="<fmt:message key="
                                                                       search-advanced.DateRange.label.Year" />" />
                                                            </div>
                                                        </c:when>
                    ...
                    <c:otherwise>
                        <div class="EXLAdvancedSearchFormRow">
                            <div class="EXLAdvancedSearchFormRowInlineInput form-group">
                                <c:forEach var="input" items="${queryTerm.inputs}" varStatus="status">
                                    <label for="exlidInput_${input.componentType}_"><fmt:message
                                            key="search-advanced.${input.componentType}.label.${status.index}"/></label>
...
<div class="EXLSearchFieldRibbonFormSubmitSearch pull-right">
    <input id="goButton" name="Submit" type="submit" value='<fmt:message key="link.title.search.search"/>'
...
<div class="EXLSearchFieldRibbonFormLinks">
         ...
         <a href="${fn:escapeXml(simple_search_url)}" title="${basic_search_title}"
            class="btn btn-default btn-sm"><fmt:message key="label.simple_search"/></a>
       </span>
</div>
</fieldset>
