...
<c:if test="${not empty errorString and fn:contains(errorString, 'on campus to view this record')}">
    <div class="EXLSystemFeedback" id="exlidHeaderSystemFeedbackFullAuthorization">
        <strong>
            <fmt:message key="search.dlsearch.error.authorization">
...
<div class="EXLSummaryContainer"><a name="${result.id}" id="${result.id}" class="EXLResultRecordId"></a>
    ...
    <div class="EXLSummaryFields">
        <c:set var="recordTitle"><prm:fields fields="${displayForm.resultView[0]}" result="${result}"
                                             fieldDelims="${displayForm.displayFieldsDelimiters[0]}"/></c:set>
        <c:set var="recordAuthor"><prm:fields fields="${displayForm.resultView[1]}" result="${result}"
                                              fieldDelims="${displayForm.displayFieldsDelimiters[1]}"/></c:set>
        ...
        <c:set var="resultDetailsThirdLine"><prm:fields fields="${c_value_is_part_of}" result="${result}"
                                                        fieldDelims=" "/></c:set>
        ...
        <c:set var="fourthLineContent"><prm:fields fields="${displayForm.resultView[2]}" result="${result}"
                                                   fieldDelims="${displayForm.displayFieldsDelimiters[2]}"/></c:set>
        ...
        <cite class="EXLResultFRBR">
            ...
            <a class="EXLBriefResultsDisplayMultipleLink" target="_parent" href="${fn:escapeXml(frbrUrl)}">
                <c:choose>
                    ...
                    <c:otherwise>
                        <fmt:message key="frbrversion.editions.prefix"/>&nbsp;${frbrnum}&nbsp;<fmt:message
                            key="frbrversion.editions"/>
                    </c:otherwise>
                </c:choose>
            </a>
            <span class="EXLResultBgRtlFRBR"></span>
...
<div>
<!-- result.id: ${result.id} tabState: ${searchForm.recordTabs[result.id]} tabOrder: ${searchForm.recordTabs[result.id].tabsOrder} -->
...
<c:if test="${tab=='viewonline' && not empty tabState.viewOnlineTab}">
    <li id="exlidResult0-ViewOnlineTab"
        class="EXLViewOnlineTab EXLResultTab ${specialTabClass} ${ (renderForm.tabs[0] eq 'viewOnlineTab') ? 'EXLResultSelectedTab':''} ${tabState.viewOnlineTab.iconCode}">
        <c:choose>
...
<c:if test="${tab=='getit_link1' && not empty tabState.requestTab}">
    <li id="exlidResult0-RequestTab"
        class="EXLRequestTab EXLResultTab ${specialTabClass} ${ (renderForm.tabs[0] eq 'requestTab') ? 'EXLResultSelectedTab':''} ${tabState.requestTab.iconCode}">
        <c:choose>
...
I changed its name so we don't try to call its setter --%>
        <c:set var="taburl"
               value="${fn:replace(taburl, 'ilsApiId=', 'tmp=')}"/> <%-- IMPORTANT! Don't remove. See comment above --%>
...
<c:if test="${tab=='locations' && not empty tabState.locationsTab}">
    <li id="exlidResult0-LocationsTab"
        class="EXLLocationsTab EXLResultTab ${ (renderForm.tabs[0] eq 'locationsTab') ? 'EXLResultSelectedTab ':''} ${specialTabClass}">
        <a href="${fn:escapeXml(displayURL)}&amp;tabs=locationsTab&amp;gathStatTab=true" title=""><fmt:message
                key="${tabState.locationsTab.label}"/></a>
        <!-- rum statistics -->
...
<c:if test="${tab=='details' && not empty tabState.detailsTab}">
    <li id="exlidResult0-DetailsTab"
        class="EXLDetailsTab EXLResultTab ${ (empty renderForm.tabs or renderForm.tabs[0] eq 'detailsTab') ? 'EXLResultSelectedTab ':''} ${specialTabClass}">
        <a href="${fn:escapeXml(displayURL)}&amp;tabs=detailsTab&amp;gathStatTab=true" title=""><fmt:message
                key="${tabState.detailsTab.label}"/></a>
        <!-- rum statistics -->
...
<c:if test="${tab=='reviewsandtags' && not empty tabState.tagsReviewsTab}">
    <li id="exlidResult0-ReviewsTab"
        class="EXLReviewsTab EXLResultTab ${ (renderForm.tabs[0] eq 'tagreviewsTab') ? 'EXLResultSelectedTab ':''} ${specialTabClass}">
        <a href="${fn:escapeXml(displayURL)}&amp;tabs=tagreviewsTab&amp;gathStatTab=true" title=""><fmt:message
                key="${tabState.tagsReviewsTab.label}"/></a>
        <!-- rum statistics -->
...
<c:if test="${tab=='recommendations' && not empty tabState.recommendationsTab}">
    ...
    <li id="exlidResult0-RecommendTab"
        class="EXLRecommendTab EXLResultTab ${ (renderForm.tabs[0] eq 'recommendTab') ? 'EXLResultSelectedTab ':''} ${specialTabClass}">
        <a href="${fn:escapeXml(displayURL)}&amp;tabs=recommendTab&amp;gathStatTab=true" title="${linkTitle}"
           id="exlidHref0"><fmt:message key="${tabState.recommendationsTab.label}"/></a>
        <!-- rum statistics -->
...
<c:if test="${tab=='getit_link2' && not empty tabState.moreTab}">
    <li id="exlidResult0-MoreTab"
        class="EXLMoreTab EXLResultTab ${ (renderForm.tabs[0] eq 'moreTab') ? 'EXLResultSelectedTab ':''} ${specialTabClass} ${tabState.moreTab.iconCode}">
        <c:choose>
...
<c:if test="${tab=='citations' && not empty tabState.citationsTab}">
    <li id="exlidResult0-CitationsTab" style="display:none;"
        class="EXLCitationsTab EXLServiceConditionalTab EXLResultTab ${ (empty renderForm.tabs or renderForm.tabs[0] eq 'citationsTab') ? 'EXLResultSelectedTab ':''} ${specialTabClass}">
        ...
        <a href="${fn:escapeXml(displayURL)}&amp;tabs=conditionalTab&amp;gathStatTab=true&amp;tabRealType=citations"
           title="">
...
<c:if test="${tab=='browseshelf' && not empty tabState.browseshelfTab}">
    <li id="exlidResult0-BrowseshelfTab"
        class="EXLBrowseshelfTab EXLServiceBrowseshelfTab EXLResultTab ${ (empty renderForm.tabs or renderForm.tabs[0] eq 'browseshelfTab') ? 'EXLResultSelectedTab ':''} ${specialTabClass}">
...
<c:if test="${tab=='onlinereviews' && not empty tabState.onlinereviewsTab}">
    <li id="exlidResult0-OnlinereviewsTab" style="display:none;"
        class="EXLOnlinereviewsTab EXLServiceConditionalTab EXLResultTab ${ (empty renderForm.tabs or renderForm.tabs[0] eq 'onlinereviewsTab') ? 'EXLResultSelectedTab ':''} ${specialTabClass}">
        <input class="EXLServiceConditionalTabService" type="hidden" value="${tabState.citationsTab.serviceName}"/>
        <input class="EXLServiceConditionalTabRecord" type="hidden" value="${result.id}"/>
        <a href="${fn:escapeXml(displayURL)}&amp;tabs=conditionalTab&amp;gathStatTab=true&amp;tabRealType=onlinereviews"
           title="">
        ...
        <prm:boomerang id="onlinereviews_${0}" boomForm="${displayForm}" pageId="brief" opId="onlinereviews"
                       resultDoc="${displayForm.searchResultFullDoc[0]}"
                       type="onlinereviews" delivery="${displayForm.delivery[0]}" noOther="false"
                       index="${param.indx}"/>
        <!-- end rum statistics -->
    </li>
    <c:set var="noFirstTab" value="false"/>
...
<div id="exlidResult${resultStatusIndex}-TabContainer-viewOnlineTab"
     class="EXLResultTabContainer EXLContainer-viewOnlineTab ${ (renderForm.tabs[0] eq 'viewOnlineTab') ? '' :'EXLResultTabContainerClosed' }">
    <c:if test="${renderForm.tabs[0] eq 'viewOnlineTab'}">
        <prm:renderTabContent live="true" form="${viewOnlineForm}" tabKey="viewOnlineTab" recordId="${result.id}"
                              recordElementId="${resultStatus.index}" popOutUrl="${popOutUrl}"/>
    </c:if>
</div>
<div id="exlidResult${resultStatusIndex}-TabContainer-requestTab"
     class="EXLResultTabContainer EXLContainer-requestTab ${ (renderForm.tabs[0] eq 'requestTab') ? '' :'EXLResultTabContainerClosed' }">
    <c:if test="${renderForm.tabs[0] eq 'requestTab'}">
        <prm:renderTabContent live="true" form="${requestTabForm}" tabKey="requestTab" recordId="${result.id}"
                              recordElementId="${resultStatus.index}" popOutUrl="${popOutUrl}"/>
    </c:if>
</div>
<div id="exlidResult${resultStatusIndex}-TabContainer-locationsTab"
     class="EXLResultTabContainer EXLContainer-locationsTab ${ (renderForm.tabs[0] eq 'locationsTab') ? '' :'EXLResultTabContainerClosed' }">
    <c:if test="${renderForm.tabs[0] eq 'locationsTab'}">
        <prm:renderTabContent live="true" form="${locationsTabForm}" tabKey="locationsTab" recordId="${result.id}"
                              recordElementId="${resultStatus.index}" popOutUrl="${popOutUrl}"/>
    </c:if>
</div>
<div id="exlidResult${resultStatusIndex}-TabContainer-detailsTab"
     class="EXLResultTabContainer EXLContainer-detailsTab ${ (empty renderForm.tabs or renderForm.tabs[0] eq 'detailsTab') ? '' :'EXLResultTabContainerClosed' }">
    <c:if test="${empty renderForm.tabs or renderForm.tabs[0] eq 'detailsTab'}">
        <c:catch var="defaultDetailsTabLoadAttempt">
            <prm:renderTabContent live="true" form="${detailsForm}" tabKey="detailsTab" recordId="${result.id}"
                                  recordElementId="${resultStatus.index}" popOutUrl="${popOutUrl}"/>
        </c:catch>
...
<div id="exlidResult${resultStatusIndex}-TabContainer-tagreviewsTab"
     class="EXLResultTabContainer EXLContainer-tagreviewsTab ${ (renderForm.tabs[0] eq 'tagreviewsTab') ? '' :'EXLResultTabContainerClosed' }">
    <c:if test="${renderForm.tabs[0] eq 'tagreviewsTab'}">
        <prm:renderTabContent live="true" form="${tagReviewsForm}" tabKey="tagreviewsTab" recordId="${result.id}"
                              recordElementId="${resultStatus.index}" popOutUrl="${popOutUrl}"/>
    </c:if>
</div>
<div id="exlidResult${resultStatusIndex}-TabContainer-recommendTab"
     class="EXLResultTabContainer EXLContainer-recommendTab ${ (renderForm.tabs[0] eq 'recommendTab') ? '' :'EXLResultTabContainerClosed' }">
    <c:if test="${renderForm.tabs[0] eq 'recommendTab'}">
        <prm:renderTabContent live="true" form="${recommendationsForm}" tabKey="recommendTab" recordId="${result.id}"
                              recordElementId="${resultStatus.index}" popOutUrl="${popOutUrl}"/>
    </c:if>
</div>
<div id="exlidResult${resultStatusIndex}-TabContainer-moreTab"
     class="EXLResultTabContainer EXLContainer-moreTab ${ (renderForm.tabs[0] eq 'moreTab') ? '' :'EXLResultTabContainerClosed' }">
    <c:if test="${renderForm.tabs[0] eq 'moreTab'}">
        <prm:renderTabContent live="true" form="${moreForm}" tabKey="moreTab" recordId="${result.id}"
                              recordElementId="${resultStatus.index}" popOutUrl="${popOutUrl}"/>
    </c:if>
</div>
<div id="exlidResult${resultStatusIndex}-TabContainer-citationsTab"
     class="EXLResultTabContainer EXLContainer-citationsTab " style="display:none;">

</div>
<div id="exlidResult${resultStatusIndex}-TabContainer-onlinereviewsTab"
     class="EXLResultTabContainer EXLContainer-onlinereviewsTab " style="display:none;">

</div>
<div id="exlidResult${resultStatusIndex}-TabContainer-browseshelfTab"
     class="EXLResultTabContainer EXLContainer-browseshelfTab ${ (renderForm.tabs[0] eq 'browseshelfTab') ? '' :'EXLResultTabContainerClosed' }">
    <c:if test="${renderForm.tabs[0] eq 'browseshelfTab'}">
        <prm:renderTabContent live="true" form="${browseshelfTabForm}" tabKey="browseshelfTab" recordId="${result.id}"
                              recordElementId="${resultStatus.index}" popOutUrl="${popOutUrl}"/>
    </c:if>
</div>
<%--
...
<script type="text/javascript">
    ...
    addLoadEvent(function () {
        checkRecommendation('${result.id}', '${renderForm.tabs[0]}', '${fromEshelf}');
    });
    //}
...
<div class="EXLResultsHeader EXLResultsFooter">
    <c:if test="${not empty displayForm}">
        ...
        <c:url value="search.do?pag=bak&indx=${backIndex}&ct=&viewAllItemsClicked=false&selectedLocation=&${displayForm.reqDecQry}&doc=${result.id}"
               var="backURL"/>
        <c:if test="${form.fn != 'display' and empty errorString}">
                ...
                <c:choose>
                    <c:when test="${(displayForm.mode != null) &&(displayForm.mode eq 'BrowseSearch') && fn:contains(displayForm.searchField, 'callnumber')}">
                     ...
                     <c:if test="${fn:contains(displayForm.searchField, 'callnumber')}"> --%>
                        ...
                        <%--                                            </c:if>
                        --%> <a id="backToResultsBtn" class="btn btn-default" href="${fn:escapeXml(BackToBrowseSearch)}"
         title="<fmt:message key='${browseMessageSuffix}' />">
                        <span class="glyphicon glyphicon-chevron-left"></span> <fmt:message
                            key="${browseMessageSuffix}"/>
                    </a>
                    </c:when>

                    <c:otherwise>
                        <a id="backToResultsBtn" class="btn btn-default" href="${fn:escapeXml(backURL)}"
                           target="_parent" title='<fmt:message key="link.title.deatiles.back"/>'><fmt:message
                                key="fulldisplay.deatiles.back"/></a>
                    </c:otherwise>
                </c:choose>
            </div>
        </c:if>
            ...
            <ul class="pager">
                <c:if test="${displayForm.indx!='1'}">
                    ...
                    <li class="EXLPrevious previous">
                        <a class="EXLPrev" target="_parent" title='<fmt:message key="link.title.results.prev"/>'
                           href="${fn:escapeXml(paginationPrevURL)}">&larr; <fmt:message key='results.prev'/></a>
                    </li>

                </c:if>

                <li class="EXLDisplayedCountTitle text-muted"><strong><fmt:message key='fulldisplay.deatiles.result'/>&nbsp;
                </strong></li>
                ...
                <c:if test="${displayForm.indx!=displayForm.searchResult.numberOfResults}">
                    ...
                    <li class="next">
                        <a class="EXLNext" target="_parent" title='<fmt:message key="link.title.results.next"/>'
                           href="${fn:escapeXml(paginationNextURL)}"><fmt:message key='results.next'/> &rarr;</a>
                    </li>
                </c:if>
            </ul>
        </div>
...
<prm:boomerang id="title" boomForm="${displayForm}" pageId="brief"
               opId="${opId}" resultDoc="${displayForm.searchResultFullDoc[0]}" type="title,${renderForm.tabs[0]}"
               delivery="${displayForm.delivery[0]}" noOther="false" index="${param.indx}"/>
