                        ...
                        <c:forEach items="${facet.facetValues}" var="facetValue" varStatus="status" end="4">
                            ...
                            <li class="EXLTopLevelFacetItem ${topLevelFacetSpecialClass}">
                                <a href="${fn:escapeXml(facet_url)}"><fmt:message
                                    ...
                                    <span class="EXLParenthesis badge pull-right">(<fmt:formatNumber
            ...
            <div class="EXLRefinementsList">
                ...
                <c:forEach items="${form.refinements}" var="refine" varStatus="refineStatus">
                    ...
                    <c:set var="displayValue"
                           value="${fmt:clearLangSuffix(fn:escapeXml(refine.facetField), fn:escapeXml(refine.fvalue))}"/>
                    ...
                    <c:url var="tmp3" value="${fn:replace(tmp2, 'lastPagIndx', 'indx')}"/>
                    <c:url var="removeRefinementUrl" value="${fn:replace(tmp3, 'lastPag', 'pag')}"/>
                    ...
                    <a href="${removeRefinementUrl}" title="${removeFacetTitle}" class="${refClass} btn btn-success">
                        <c:if test="${fn:contains(refine.facetField,c_facet_localX)}">
                            <c:set var="displayValue">
                                <fmt:message
                                        key="facets.facet.${fn:escapeXml(refine.facetField)}.${fn:escapeXml(refine.fvalue)}"
                                        prefix="facets.facet.${fn:escapeXml(refine.facetField)}"/>
                            </c:set>
                        </c:if>
