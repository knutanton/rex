    ...
    <ul class="pagination">
        <c:if test="${form.searchResult.numberOfResults>bulkSize || form.indx>1}">
            ...
            <c:if test="${form.indx!='1'}">
                ...
                <li>
                    <a href="${fn:escapeXml(paginationPrevURL)}" title="${previousTitle}"
                       class="EXLPrevious EXLBriefResultsPaginationLinkPrevious  EXLBriefResultsPagination">&laquo;</a>
                </li>


            </c:if>
            <c:if test="${form.presentPaging}">
                <c:forEach begin="${form.plsp.firstPagePresented}" end="${form.plsp.lastPagePresented}"
                           varStatus="loop">
                    <c:if test="${loop.index == 1 && form.indx!='1'}">
                        ...
                        <li>
                            <a href="${fn:escapeXml(paginationPage)}"
                               title="<fmt:message key='link.title.results.page'><fmt:param value="${loop.index}"
                            /></fmt:message>" class="EXLPrevious EXLBriefResultsPaginationLinkPrevious
                            EXLBriefResultsPagination">${loop.index}</a>
                        </li>
                    </c:if>
                    ...
                    <c:if test="${loop.index == form.plsp.currentPageNumber}">
                        <span class="EXLHide hide"><fmt:message key='results.rescount'/></span>
                        <li class="active">
            ...
            <c:if test="${(form.indx-1)+bulkSize<form.searchResult.numberOfResults}">
                ...
                <li><a href="${fn:escapeXml(paginationNextURL)}" title="${nextTitle}"
                       class="EXLNext EXLBriefResultsPaginationLinkNext EXLBriefResultsPagination">&raquo;</a></li>
