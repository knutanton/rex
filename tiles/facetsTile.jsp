  ...
  <c:if test="${form.mode == 'Basic' or (form.mode == 'Advanced' and !form.showFullText)}">
	...
	<div class="EXLFacetList EXLTopLevelFacetList">
           <div class="EXLFacetContainer">
               <h3><fmt:message key="expand.options"/></h3>
		...
		<c:otherwise>
			...
			<li class="EXLFacet EXLSearchAndLinkResource">
				<c:choose>
					...
					<c:otherwise>
						<a id="salLink${sal_resultStatus.index}" title="${titleStrArr[0]}" href="${fn:escapeXml(sal_result.values[c_value_source][0])}" target="popup">
			  	...
			  	<div id="exlidFacet${facetIndex.index}" class="EXLFacetContainer">
					...
					<c:choose>
						...
						<c:otherwise>
							...
							<h4 class="EXLFacetTitleLabelPHolder${facetField}"><fmt:message key="facets.facet.${facetField}"/>&nbsp;</h4>
						</c:otherwise>
					</c:choose>
				    ...
				    <ol id="exlidFacetSublist${facetIndex.index}" class="EXLFacetsList EXLFacetsListPreview">
						...
						<c:forEach items="${facet.facetValues}" var="facetValue" varStatus="status" >
					      ...
					      <li class="EXLFacet ${hiddenFacet}">
								<a href="${facet_url}" id="exlidFacet${facetIndex.index}-${status.index}">${displayValue}</a>&nbsp;
