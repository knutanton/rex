...
<%@ include file="/javascript/TitleSearch.js" %>
    ...
    <html:form styleId="searchForm" method="post" styleClass="EXLSearchForm" action="/action/search.do?${action_func}=go&amp;ct=search" enctype="application/x-www-form-urlencoded; charset=utf-8" onsubmit="if(isRemoteSearch()){doPleaseWait();}if(window.manualFormSubmit){manualFormSubmit(this.id);return false;}" target="${resloc}">
                        ...
                        <ul id="exlidSearchTabs" class="EXLTabs nav nav-tabs">
                            <c:forEach items="${primoView.avilableTabs}" var="menu_item"  varStatus="status">
                                ...
                                <li id="exlidTab${status.index}" class="EXLSearchTab ${(searchForm.tab == status.current )?'EXLSearchTabSelected, active':''}">
                                    ...
                                    <span id="defaultScope${menu_item}" style='display:none'><fmt:message key='scopes.option.${defScopeId}' /></span>
                                    <%-- where is this used? (-hj) --%>
                                    <a href="${tab_url}&tab=${menu_item}" data-toggle="tab" class="EXLSearchTabTitle EXLSearchTabLABEL${menuItemLabel}" onclick="getSearchField(this,'${searchForm.mode}'); delay4Remote('${isRemote}','${searchForm.tabsRemote} ','${menu_item}')" title="<fmt:message key="tabbedmenu.${menu_item}.tooltip" />">
                                        ...
                                        <c:if test="${searchForm.location[queryTermStatus.index]=='L'}">
                                            <c:choose>
                                                <c:when test="${input.componentType == c_ctype_freetext}">
                                                    <label class="EXLHide collapse" for="input_${input.id}">
