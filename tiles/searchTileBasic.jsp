                ...
                <ul id="exlidSearchTabs" class="EXLTabs nav nav-tabs">
                    <c:forEach items="${primoView.avilableTabs}" var="menu_item"  varStatus="status">
                        ...
                        <li id="exlidTab${status.index}" class="EXLSearchTab ${(searchForm.tab == status.current )?'EXLSearchTabSelected, active':''}">
                            ...
                            <a href="${fn:escapeXml(tabUrl)}" class="EXLSearchTabTitle EXLSearchTabLABEL${menuItemLabel}" target="${resloc}" onclick="${autoSearchScript} delay4Remote('${isRemote}','${searchForm.tabsRemote} ','${menu_item}')" title="<fmt:message key="tabbedmenu.${menu_item}.tooltip" />">
...
<%-- end tabs handling --%>
        ...
        <div class="EXLSearchFieldRibbon ${enlargeSearchFieldHideScope} ">
            <div class="EXLSearchFieldRibbonFormFields form-group">
                <div class="EXLSearchFieldRibbonFormSearchFor input-group input-group-lg">
                    ...
                    <input name="vl(${searchForm.queryTerms[0].inputs[0].id})" class="${fullSearchBoxClass} form-control" value='${fn:escapeXml(searchForm.values[searchForm.queryTerms[0].inputs[0].id])}' id="search_field" type="text" accesskey="s" />

                    <div class="EXLSearchFieldRibbonFormSubmitSearch input-group-btn">
                        <c:if test="${empty sbutton }">
                            <c:set var="sbutton" value="submit"/>
                        </c:if>
                        <button id="goButton" type="submit" value="<fmt:message key='link.title.search.search'/>" class="${sbutton} btn btn-primary" accesskey="g"><fmt:message key='link.title.search.search'/></button>
                    </div>
                </div>
        ...
        <c:if test="${searchForm.showBrowseLink }">
            ...
            <div class="EXLSearchFieldRibbonBrowseSearchLink">
                <a class="${browseAlign}" title="<fmt:message key='browse.browsesearch'/>" href="${fn:escapeXml(browse_search_url)}">
