...
<div class="container">
    <ul class="nav nav-pills">
		...
		<c:set var="entryTab" value="1"/>
                <a title="${link_title}" href="#" class="EXLMainMenuITEMFindInDB" onclick="boomCallToRum('FindDbStat',false);openPrimoLightBox('searchDB','loadSearchDbPage','searchDbXml','');return false;">
            ...
            <c:if test="${!(menu.label=='tags' && is_display_tags=='N')}">
                <li id="exlidMainMenuItem${count}" class="EXLMainMenuItem${menu.label=='atoz' && form.alma ?'Selected':((count==0)? ' EXLFirstItem':'') }">
                  ...
                  --%>
                    ...
                    <c:choose>
                        <c:when test="${menu.target=='_blank'}">
                            <!-- SFX A-Z should be opened in a new window -->
                            <a title="${atoz_title}" href="${fn:escapeXml(menu_link)}" target="popup" onclick="openWindow(this.href, this.target, 'top=100,left=50,width=600,height=500,resizable=1,scrollbars=1'); return false;" class="${(sessionScope.menuitem == status.index)?'selected ':''} ${(sessionScope.menuitem == status.index)?'EXLMainMenuSelected':''} EXLMainMenuITEMATOZ">
                                <fmt:message key='mainmenu.label.${menu.label}'/>
                            </a>
                        </c:when>
                        <c:otherwise>    <!-- Alma A-Z should be opened in the current window -->
                            <a title="${atoz_title}" href="${fn:escapeXml(menu_link)}" class="${(sessionScope.menuitem == status.index)?'selected ':''} ${(sessionScope.menuitem == status.index)?'EXLMainMenuSelected':''} EXLMainMenuITEMATOZ">
                                <fmt:message key='mainmenu.label.${menu.label}'/>
                            </a>
                            <c:if test="${form.alma}">
                                <a title="<fmt:message key='alma.atoz.back_to_simple_search' />" href="search.do?fromTop=true&amp;fromPreferences=${form.fromPreferences}&amp;fromEshelf=${form.fromEshelf}&amp;vid=${sessionScope.vid}" class="${(sessionScope.menuitem == status.index)?'selected ':''} ${(sessionScope.menuitem == status.index)?'EXLMainMenuSelected':''} EXLMainMenuITEMATOZ EXLMainMenuITEMATOZClose">
                        ...
                        <c:otherwise>
                            ...
                            <a title="${link_title}" href="${fn:escapeXml(menu_link)}&amp;fromTop=true&amp;fromPreferences=${form.fromPreferences}&amp;fromEshelf=${form.fromEshelf}&amp;vid=${sessionScope.vid}" class="${(sessionScope.menuitem == status.index)?'selected ':''} ${(sessionScope.menuitem == status.index)?'EXLMainMenuSelected ':''}EXLMainMenuITEM${menu.label}" ${mtarget }>
                                <fmt:message key='mainmenu.label.${menu.label}'/>
                            </a>

                            </c:if>
                            <c:if test="${!(menu.label=='tags' || menu.label=='library_search')}">
                                <a title="${link_title}" href="${fn:escapeXml(menu_link)}" class="${(sessionScope.menuitem == status.index)?'selected ':''} ${(sessionScope.menuitem == status.index)?'EXLMainMenuSelected ':''}EXLMainMenuITEM${menu.label}" ${mtarget }>
                                    <fmt:message key='mainmenu.label.${menu.label}'/>
                                </a>
                            </c:if>
                        </c:otherwise>
        ...
        <li id="exlidMainMenuItem${count+1}" class="EXLMainMenuItem EXLLastItem">
            ...
            <a title="${link_title}" href="${fn:escapeXml(helpURL)}" class="EXLMainMenuITEMHelp" target="popup" onclick="openWindow(this.href, this.target, 'top=100,left=50,width=600,height=500,resizable=1,scrollbars=1'); return false;">
            ...
            <c:if test="${form.interfaceLangSize > 1}">
                ...
                <ul class="nav nav-pills">
                    <li class="dropdown">
                        ...
                        <c:set var="lang_title">
                            <fmt:message key="link.title.select.interface.language"/>
                        </c:set>

                        <ul class="dropdown-menu">

                            <!--remove EXLLanguageMenuHide to show menu items-->
